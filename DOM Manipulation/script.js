...
document.getElementById('show-img').addEventListener('click', function() {
    console.log(12);
    const imageBox = document.getElementById('task-11')
    const img = document.createElement('div')
    img.innerHTML = `<img src="1.jpg" alt="" style="width: 50%; height: 50%; margin-top: 20px" >`
    console.log(img);
...
document.getElementById('link-12').addEventListener('mouseover', function() {
    const boldie = document.getElementsByTagName('strong')
    for(let i = 0; i < boldie.length; i++) {
        boldie[i].style.backgroundColor = 'yellow'
    }
})
...
document.body.addEventListener(onload, () => {
    ...
    document.getElementById('div-13').innerHTML = "<h1>Width:" + w + "Height: " + h + "</h1>"
})

document.body.addEventListener(onresize, () => {
    const w = document.documentElement.clientWidth
    const h = document.documentElement.clientHeight
